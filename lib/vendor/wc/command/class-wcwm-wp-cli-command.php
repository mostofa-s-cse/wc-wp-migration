...
if ( class_exists( 'WP_CLI_Command' ) ) {
	class Wcwm_WP_CLI_Command extends WP_CLI_Command {
		/**
		 ...
		 *
		 * Example:
		 * $ wp wcwm backup --replace "wp" "WordPress"
		 * Backup in progress...
		 * Backup complete.
		 * Backup location: /repos/migration/wp/wp-content/wcwm-backups/migration-wp-20170913-095743-931.wpress
		 *
		 * @subcommand backup
		 */
		public function backup( array $args, array $assoc_args ) {
			if ( is_multisite() ) {
				WP_CLI::error( __( 'WPI CLI is not supported in Multisite mode. Please use web interface to create a backup.', WCWM_PLUGIN_NAME ) );
			}

			if ( ! is_dir( WCWM_STORAGE_PATH ) ) {
				if ( ! mkdir( WCWM_STORAGE_PATH ) ) {
					WP_CLI::error_multi_line( array(
						sprintf( __( 'All in One WP Migration is not able to create <strong>%s</strong> folder.', WCWM_PLUGIN_NAME ), WCWM_STORAGE_PATH ),
						__( 'You will need to create this folder and grant it read/write/execute permissions (0777) for the All in One WP Migration plugin to function properly.', WCWM_PLUGIN_NAME ),
					) );
					exit;
				}
			}
			...
			if ( ! is_dir( WCWM_BACKUPS_PATH ) ) {
				if ( ! mkdir( WCWM_BACKUPS_PATH ) ) {
					WP_CLI::error_multi_line( array(
						sprintf( __( 'All in One WP Migration is not able to create <strong>%s</strong> folder.', WCWM_PLUGIN_NAME ), WCWM_BACKUPS_PATH ),
						__( 'You will need to create this folder and grant it read/write/execute permissions (0777) for the All in One WP Migration plugin to function properly.', WCWM_PLUGIN_NAME ),
					) );
					exit;
				}
			...
			if ( isset( $assoc_args['list'] ) ) {
				...
				$backups->setHeaders( array(
					'name' => __( 'Backup name', WCWM_PLUGIN_NAME ),
					'date' => __( 'Date created', WCWM_PLUGIN_NAME ),
					'size' => __( 'Size', WCWM_PLUGIN_NAME ),
				) );
				...
				foreach ( $model->get_files() as $backup ) {
					$backups->addRow( array(
						...
						'date' => sprintf( __( '%s ago', WCWM_PLUGIN_NAME ), human_time_diff( $backup['mtime'] ) ),
			...
			try {
				...
				add_filter( 'wcwm_completed_timeout', '__return_zero' );

				// Remove filters
				remove_filter( 'wcwm_export', 'Wcwm_Export_Clean::execute', 300 );

				// Run filters
				$params = apply_filters( 'wcwm_export', $params );
			...
			WP_CLI::log( __( 'Backup complete.', WCWM_PLUGIN_NAME ) );
			WP_CLI::log( sprintf( __( 'Backup location: %s', WCWM_PLUGIN_NAME ), wcwm_backup_path( $params ) ) );
		}
		 ...
		 *
		 * Example:
		 * $ wp wcwm restore migration-wp-20170913-095743-931.wpress
		 * Restore in progress...
		 * Restore complete.
		 *
		 * @subcommand restore
		...
		public function restore( array $args, array $assoc_args ) {
			if ( is_multisite() ) {
				WP_CLI::error( __( 'WPI CLI is not supported in Multisite mode. Please use web interface to restore a backup.', WCWM_PLUGIN_NAME ) );
			}
			...
			$params = array();
			if ( isset( $args[0] ) && is_file( wcwm_backup_path( array( 'archive' => $args[0] ) ) ) ) {
				$params = array(
					'storage'              => wcwm_storage_folder(),
					'archive'              => $args[0],
					'wcwm_manual_restore' => true,
				);
			} else {
				WP_CLI::error_multi_line( array(
					__( 'A valid backup name must be provided in order to proceed with the restore process.', WCWM_PLUGIN_NAME ),
					__( 'Example: wp wcwm restore migration-wp-20170913-095743-931.wpress', WCWM_PLUGIN_NAME ),
				) );
				exit;
			}
			...
			try {
				...
				// Remove filters
				remove_filter( 'wcwm_import', 'Wcwm_Import_Upload::execute', 5 );
				remove_filter( 'wcwm_import', 'Wcwm_Import_Confirm::execute', 100 );
				remove_filter( 'wcwm_import', 'Wcwm_Import_Clean::execute', 400 );

				// Run filters
				$params = apply_filters( 'wcwm_import', $params );
...
if ( class_exists( 'WP_CLI' ) ) {
	WP_CLI::add_command( 'wcwm', 'Wcwm_WP_CLI_Command', array( 'short_desc' => 'WC WP Migration Command' ) );
}
