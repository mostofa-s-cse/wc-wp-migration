...
class Wcwm_Compressor extends Wcwm_Archiver {
			...
			if ( ( $block = $this->get_file_block( $file_name, $new_file_name ) ) ) {
				...
				if ( $file_offset === 0 ) {
					if ( ( $file_bytes = @fwrite( $this->file_handle, $block ) ) !== false ) {
						if ( strlen( $block ) !== $file_bytes ) {
							throw new Wcwm_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write header to file. File: %s', $this->file_name ) );
						}
					} else {
						throw new Wcwm_Not_Writable_Exception( sprintf( 'Unable to write header to file. File: %s', $this->file_name ) );
					}
				}
						...
						if ( ( $file_content = @fread( $file_handle, 512000 ) ) !== false ) {
							if ( ( $file_bytes = @fwrite( $this->file_handle, $file_content ) ) !== false ) {
								if ( strlen( $file_content ) !== $file_bytes ) {
									throw new Wcwm_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write content to file. File: %s', $this->file_name ) );
								}
							} else {
								throw new Wcwm_Not_Writable_Exception( sprintf( 'Unable to write content to file. File: %s', $this->file_name ) );
							}
				...
				if ( ( $block = $this->get_file_size_block( $file_offset ) ) ) {
					...
					if ( @fseek( $this->file_handle, - $file_offset - 4096 - 12 - 14, SEEK_CUR ) === -1 ) {
						throw new Wcwm_Not_Seekable_Exception(
							'Your PHP is 32-bit. In order to export your file, please change your PHP version to 64-bit and try again. ' .
							'<a href="https://help.wc.com/knowledgebase/php-32bit/" target="_blank">Technical details</a>'
						);
					}
					...
					if ( ( $file_bytes = @fwrite( $this->file_handle, $block ) ) !== false ) {
						if ( strlen( $block ) !== $file_bytes ) {
							throw new Wcwm_Quota_Exceeded_Exception( sprintf( 'Out of disk space. Unable to write size to file. File: %s', $this->file_name ) );
						}
					} else {
						throw new Wcwm_Not_Writable_Exception( sprintf( 'Unable to write size to file. File: %s', $this->file_name ) );
					}
