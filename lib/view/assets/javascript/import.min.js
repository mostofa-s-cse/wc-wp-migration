...
jQuery(document).ready(function ($) {
  ...
  $('.wcwm-expandable > div.wcwm-button-main').on('click', function () {
    $(this).parent().toggleClass('wcwm-open');
  });
});

global.Wcwm = jQuery.extend({}, global.Wcwm, { FileUploader: FileUploader, Feedback: Feedback, Report: Report, Import: Import });
...
FileUploader.prototype.init = function () {
	...
	var formElement = $('#wcwm-import-form');
	var selectElement = $('#wcwm-import-file');
	var dropElement = $('#wcwm-drag-drop-area');
	...
	dropElement.on('dragenter', function (e) {
		dropElement.addClass('wcwm-drag-over');
		e.preventDefault();
	});
	...
	dropElement.on('dragleave', function (e) {
		dropElement.removeClass('wcwm-drag-over');
		e.preventDefault();
	});
...
FileUploader.prototype.upload = function (file, retries) {
	...
	var chunkSize = file.size > wcwm_uploader.chunk_size ? wcwm_uploader.chunk_size : file.size;
	var formData = this.getFormData(file, chunkSize);

	fetch(wcwm_uploader.url, {
		method: 'POST',
		credentials: 'include',
		body: formData
		...
		switch (response.status) {
			case 413:
				wcwm_uploader.chunk_size = chunkSize / 2;
				return _this2.upload(file);
			case 200:
				return response.text().then(function (text) {
					var extractedJsonText = Wcwm.Util.json(text);
		...
		var timeout = retries * 1000;
		if (retries >= 30) {
			return _this2.onError(new Error(wcwm_locale.problem_while_uploading_your_file));
		}
...
FileUploader.prototype.getFormData = function (file, chunkSize) {
	var params = wcwm_uploader.params;
...
FileUploader.prototype.checkSize = function (file) {
	var size = parseInt(wcwm_uploader.filters.wcwm_archive_size, 10);

	if (file.size > size && size !== 0) {
		throw new Error(wcwm_locale.invalid_archive_size);
	}
};

FileUploader.prototype.checkExtension = function (file) {
	var extensions = wcwm_uploader.filters.wcwm_archive_extension;
	...
	if (!matchExtension) {
		throw new Error(wcwm_locale.invalid_archive_extension);
	}
};
...
FileUploader.prototype.onBeforeUpload = function (file) {
	...
	var options = Wcwm.Util.form('#wcwm-import-form').concat({ name: 'storage', value: storage }).concat({ name: 'archive', value: file.name });

	// Set global params
	this.model.setParams(options);

	// Set multipart params
	$.extend(wcwm_uploader.params, {
		storage: storage,
		archive: file.name
	});
...
FileUploader.prototype.onError = function (error) {
	this.model.setStatus({ type: 'error', title: wcwm_locale.unable_to_import, message: error.message });
};
