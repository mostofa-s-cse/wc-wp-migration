...
class Wcwm_Export_Enumerate {
	...
	public static function execute( $params ) {
		...
		// Set progress
		Wcwm_Status::info( __( 'Retrieving a list of all WordPress files...', WCWM_PLUGIN_NAME ) );

		// Set exclude filters
		$exclude_filters = wcwm_content_filters();
		...
		if ( isset( $params['options']['no_plugins'] ) ) {
			...
			// Set exclude filters
			$exclude_filters = array_merge( $exclude_filters, wcwm_plugin_filters( $inactive_plugins ) );
		}
		...
		// Create map file
		$filemap = wcwm_open( wcwm_filemap_path( $params ), 'w' );

		// Iterate over content directory
		$iterator = new Wcwm_Recursive_Directory_Iterator( WP_CONTENT_DIR );

		// Exclude new line file names
		$iterator = new Wcwm_Recursive_Newline_Filter( $iterator );

		// Exclude uploads, plugins or themes
		$iterator = new Wcwm_Recursive_Exclude_Filter( $iterator, apply_filters( 'wcwm_exclude_content_from_export', $exclude_filters ) );

		// Recursively iterate over content directory
		$iterator = new Wcwm_Recursive_Iterator_Iterator( $iterator, RecursiveIteratorIterator::LEAVES_ONLY, RecursiveIteratorIterator::CATCH_GET_CHILD );

		// Write path line
		foreach ( $iterator as $item ) {
			if ( $item->isFile() ) {
				if ( wcwm_write( $filemap, $iterator->getSubPathName() . PHP_EOL ) ) {
					$total_files_count++;
		...
		// Set progress
		Wcwm_Status::info( __( 'Done retrieving a list of all WordPress files.', WCWM_PLUGIN_NAME ) );
