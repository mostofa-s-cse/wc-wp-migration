...
class Wcwm_Export_Config_File {
	...
	public static function execute( $params ) {
		...
		if ( isset( $params['archive_bytes_offset'] ) ) {
			...
			$archive_bytes_offset = wcwm_archive_bytes( $params );
		}
		...
		if ( isset( $params['total_package_size'] ) ) {
			...
			$total_package_size = wcwm_package_bytes( $params );
		}
		...
		// Set progress
		Wcwm_Status::info( sprintf( __( 'Archiving configuration file...<br />%d%% complete', WCWM_PLUGIN_NAME ), $progress ) );
		...
		if ( $archive->add_file( wcwm_package_path( $params ), WCWM_PACKAGE_NAME, $package_bytes_written, $package_bytes_offset ) ) {

			// Set progress
			Wcwm_Status::info( __( 'Done archiving configuration file.', WCWM_PLUGIN_NAME ) );
