...
class Wcwm_Updater {
	...
	public static function plugins_api( $result, $action = null, $args = null ) {
		...
		if ( isset( $args->slug ) && isset( $extensions[ $args->slug ] ) && $action === 'plugin_information' ) {
			...
			// Get current updates
			$updates = get_option( WCWM_UPDATER, array() );
	...
	public static function check_for_updates() {
		...
		// Get extension updates
		foreach ( Wcwm_Extensions::get() as $slug => $extension ) {
		...
		// Set new updates
		update_option( WCWM_UPDATER, $updates );
	}
		...
		foreach ( Wcwm_Extensions::get() as $slug => $extension ) {
			...
			if ( $file === $extension['basename'] ) {
				...
				if ( get_option( $extension['key'] ) ) {
					...
					// Add "Check for updates" link
					$links[] = Wcwm_Template::get_content( 'updater/check', array(
						'url' => $updater_url,
					) );
					...
					// Add modal
					$links[] = Wcwm_Template::get_content( 'updater/modal', array(
						'url'   => $updater_url,
						'modal' => $modal_index,
					) );
