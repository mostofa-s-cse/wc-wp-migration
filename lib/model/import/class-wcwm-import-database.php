...
class Wcwm_Import_Database {

	public static function execute( $params ) {
		global $wpdb;

		// Skip database import
		if ( ! is_file( wcwm_database_path( $params ) ) ) {
			return $params;
		}
		...
		$config = wcwm_read( $handle, filesize( wcwm_package_path( $params ) ) );
		...
		// Set progress
		Wcwm_Status::info( sprintf( __( 'Restoring database...<br />%d%% complete', WCWM_PLUGIN_NAME ), $progress ) );
		...
		foreach ( $blogs as $blog ) {
			...
			foreach ( $home_urls as $home_url ) {
				...
				if ( ! in_array( sprintf( "'%s'", trim( wcwm_blogsdir_path( $blog['Old']['BlogID'] ), '/' ) ), $old_replace_raw_values ) ) {
					$old_replace_raw_values[] = sprintf( "'%s'", trim( wcwm_blogsdir_path( $blog['Old']['BlogID'] ), '/' ) );
					$new_replace_raw_values[] = sprintf( "'%s'", get_option( 'upload_path' ) );
				}

				// Get sites dir Upload Path
				if ( ! in_array( sprintf( "'%s'", trim( wcwm_uploads_path( $blog['Old']['BlogID'] ), '/' ) ), $old_replace_raw_values ) ) {
					$old_replace_raw_values[] = sprintf( "'%s'", trim( wcwm_uploads_path( $blog['Old']['BlogID'] ), '/' ) );
					$new_replace_raw_values[] = sprintf( "'%s'", get_option( 'upload_path' ) );
				}
				...
				if ( defined( 'UPLOADBLOGSDIR' ) ) {
					...
					if ( ! in_array( wcwm_blogsdir_path( $blog['Old']['BlogID'] ), $old_replace_values ) ) {
						$old_replace_values[] = wcwm_blogsdir_path( $blog['Old']['BlogID'] );
						$new_replace_values[] = wcwm_blogsdir_path( $blog['New']['BlogID'] );
					}

					// Get URL encoded Upload Path
					if ( ! in_array( urlencode( wcwm_blogsdir_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = urlencode( wcwm_blogsdir_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = urlencode( wcwm_blogsdir_path( $blog['New']['BlogID'] ) );
					}

					// Get URL raw encoded Upload Path
					if ( ! in_array( rawurlencode( wcwm_blogsdir_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = rawurlencode( wcwm_blogsdir_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = rawurlencode( wcwm_blogsdir_path( $blog['New']['BlogID'] ) );
					}

					// Get JSON escaped Upload Path
					if ( ! in_array( addcslashes( wcwm_blogsdir_path( $blog['Old']['BlogID'] ), '/' ), $old_replace_values ) ) {
						$old_replace_values[] = addcslashes( wcwm_blogsdir_path( $blog['Old']['BlogID'] ), '/' );
						$new_replace_values[] = addcslashes( wcwm_blogsdir_path( $blog['New']['BlogID'] ), '/' );
					}

					// Get plain Upload Path
					if ( ! in_array( wcwm_uploads_path( $blog['Old']['BlogID'] ), $old_replace_values ) ) {
						$old_replace_values[] = wcwm_uploads_path( $blog['Old']['BlogID'] );
						$new_replace_values[] = wcwm_blogsdir_path( $blog['New']['BlogID'] );
					}

					// Get URL encoded Upload Path
					if ( ! in_array( urlencode( wcwm_uploads_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = urlencode( wcwm_uploads_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = urlencode( wcwm_blogsdir_path( $blog['New']['BlogID'] ) );
					}

					// Get URL raw encoded Upload Path
					if ( ! in_array( rawurlencode( wcwm_uploads_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = rawurlencode( wcwm_uploads_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = rawurlencode( wcwm_blogsdir_path( $blog['New']['BlogID'] ) );
					}

					// Get JSON escaped Upload Path
					if ( ! in_array( addcslashes( wcwm_uploads_path( $blog['Old']['BlogID'] ), '/' ), $old_replace_values ) ) {
						$old_replace_values[] = addcslashes( wcwm_uploads_path( $blog['Old']['BlogID'] ), '/' );
						$new_replace_values[] = addcslashes( wcwm_blogsdir_path( $blog['New']['BlogID'] ), '/' );
					}
				} else {
					...
					if ( ! in_array( sprintf( '%s/%s/', untrailingslashit( $home_url ), 'files' ), $old_replace_values ) ) {
						...
						$new_replace_values[] = wcwm_uploads_url( $blog['New']['BlogID'] );
					}

					// Get plain Upload Path
					if ( ! in_array( wcwm_blogsdir_path( $blog['Old']['BlogID'] ), $old_replace_values ) ) {
						$old_replace_values[] = wcwm_blogsdir_path( $blog['Old']['BlogID'] );
						$new_replace_values[] = wcwm_uploads_path( $blog['New']['BlogID'] );
					}

					// Get URL encoded Upload Path
					if ( ! in_array( urlencode( wcwm_blogsdir_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = urlencode( wcwm_blogsdir_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = urlencode( wcwm_uploads_path( $blog['New']['BlogID'] ) );
					}

					// Get URL raw encoded Upload Path
					if ( ! in_array( rawurlencode( wcwm_blogsdir_path( $blog['Old']['BlogID'] ) ), $old_replace_values ) ) {
						$old_replace_values[] = rawurlencode( wcwm_blogsdir_path( $blog['Old']['BlogID'] ) );
						$new_replace_values[] = rawurlencode( wcwm_uploads_path( $blog['New']['BlogID'] ) );
					}

					// Get JSON escaped Upload Path
					if ( ! in_array( addcslashes( wcwm_blogsdir_path( $blog['Old']['BlogID'] ), '/' ), $old_replace_values ) ) {
						$old_replace_values[] = addcslashes( wcwm_blogsdir_path( $blog['Old']['BlogID'] ), '/' );
						$new_replace_values[] = addcslashes( wcwm_uploads_path( $blog['New']['BlogID'] ), '/' );
					}
				...
				if ( $site_url !== $blog['New']['SiteURL'] ) {
					...
					foreach ( array( $site_url, $site_url_www_inversion ) as $url ) {
						...
						for ( $i = 0; $i < count( $old_schemes ); $i++ ) {
							...
							// Add plain Site URL
							if ( ! in_array( wcwm_url_scheme( $url, $old_schemes[ $i ] ), $old_replace_values ) ) {
								$old_replace_values[] = wcwm_url_scheme( $url, $old_schemes[ $i ] );
								$new_replace_values[] = wcwm_url_scheme( $blog['New']['SiteURL'], $new_schemes[ $i ] );
							}

							// Add URL encoded Site URL
							if ( ! in_array( urlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
								$old_replace_values[] = urlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) );
								$new_replace_values[] = urlencode( wcwm_url_scheme( $blog['New']['SiteURL'], $new_schemes[ $i ] ) );
							}

							// Add URL raw encoded Site URL
							if ( ! in_array( rawurlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
								$old_replace_values[] = rawurlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) );
								$new_replace_values[] = rawurlencode( wcwm_url_scheme( $blog['New']['SiteURL'], $new_schemes[ $i ] ) );
							}

							// Add JSON escaped Site URL
							if ( ! in_array( addcslashes( wcwm_url_scheme( $url, $old_schemes[ $i ] ), '/' ), $old_replace_values ) ) {
								$old_replace_values[] = addcslashes( wcwm_url_scheme( $url, $old_schemes[ $i ] ), '/' );
								$new_replace_values[] = addcslashes( wcwm_url_scheme( $blog['New']['SiteURL'], $new_schemes[ $i ] ), '/' );
							}
						}
				...
				if ( $home_url !== $blog['New']['HomeURL'] ) {
					...
					foreach ( array( $home_url, $home_url_www_inversion ) as $url ) {
						...
						for ( $i = 0; $i < count( $old_schemes ); $i++ ) {
							...
							if ( ! in_array( wcwm_url_scheme( $url, $old_schemes[ $i ] ), $old_replace_values ) ) {
								$old_replace_values[] = wcwm_url_scheme( $url, $old_schemes[ $i ] );
								$new_replace_values[] = wcwm_url_scheme( $blog['New']['HomeURL'], $new_schemes[ $i ] );
							}

							// Add URL encoded Home URL
							if ( ! in_array( urlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
								$old_replace_values[] = urlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) );
								$new_replace_values[] = urlencode( wcwm_url_scheme( $blog['New']['HomeURL'], $new_schemes[ $i ] ) );
							}

							// Add URL raw encoded Home URL
							if ( ! in_array( rawurlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
								$old_replace_values[] = rawurlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) );
								$new_replace_values[] = rawurlencode( wcwm_url_scheme( $blog['New']['HomeURL'], $new_schemes[ $i ] ) );
							}

							// Add JSON escaped Home URL
							if ( ! in_array( addcslashes( wcwm_url_scheme( $url, $old_schemes[ $i ] ), '/' ), $old_replace_values ) ) {
								$old_replace_values[] = addcslashes( wcwm_url_scheme( $url, $old_schemes[ $i ] ), '/' );
								$new_replace_values[] = addcslashes( wcwm_url_scheme( $blog['New']['HomeURL'], $new_schemes[ $i ] ), '/' );
							}
						}
			...
			if ( $site_url !== site_url() ) {
				...
				foreach ( array( $site_url, $site_url_www_inversion ) as $url ) {
					...
					for ( $i = 0; $i < count( $old_schemes ); $i++ ) {
						...
						if ( ! in_array( wcwm_url_scheme( $url, $old_schemes[ $i ] ), $old_replace_values ) ) {
							$old_replace_values[] = wcwm_url_scheme( $url, $old_schemes[ $i ] );
							$new_replace_values[] = wcwm_url_scheme( site_url(), $new_schemes[ $i ] );
						}

						// Add URL encoded Site URL
						if ( ! in_array( urlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
							$old_replace_values[] = urlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) );
							$new_replace_values[] = urlencode( wcwm_url_scheme( site_url(), $new_schemes[ $i ] ) );
						}

						// Add URL raw encoded Site URL
						if ( ! in_array( rawurlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
							$old_replace_values[] = rawurlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) );
							$new_replace_values[] = rawurlencode( wcwm_url_scheme( site_url(), $new_schemes[ $i ] ) );
						}

						// Add JSON escaped Site URL
						if ( ! in_array( addcslashes( wcwm_url_scheme( $url, $old_schemes[ $i ] ), '/' ), $old_replace_values ) ) {
							$old_replace_values[] = addcslashes( wcwm_url_scheme( $url, $old_schemes[ $i ] ), '/' );
							$new_replace_values[] = addcslashes( wcwm_url_scheme( site_url(), $new_schemes[ $i ] ), '/' );
						}
					}
			...
			if ( $home_url !== home_url() ) {
				...
				foreach ( array( $home_url, $home_url_www_inversion ) as $url ) {
					...
					for ( $i = 0; $i < count( $old_schemes ); $i++ ) {
						...
						if ( ! in_array( wcwm_url_scheme( $url, $old_schemes[ $i ] ), $old_replace_values ) ) {
							$old_replace_values[] = wcwm_url_scheme( $url, $old_schemes[ $i ] );
							$new_replace_values[] = wcwm_url_scheme( home_url(), $new_schemes[ $i ] );
						}

						// Add URL encoded Home URL
						if ( ! in_array( urlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
							$old_replace_values[] = urlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) );
							$new_replace_values[] = urlencode( wcwm_url_scheme( home_url(), $new_schemes[ $i ] ) );
						}

						// Add URL raw encoded Home URL
						if ( ! in_array( rawurlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) ), $old_replace_values ) ) {
							$old_replace_values[] = rawurlencode( wcwm_url_scheme( $url, $old_schemes[ $i ] ) );
							$new_replace_values[] = rawurlencode( wcwm_url_scheme( home_url(), $new_schemes[ $i ] ) );
						}

						// Add JSON escaped Home URL
						if ( ! in_array( addcslashes( wcwm_url_scheme( $url, $old_schemes[ $i ] ), '/' ), $old_replace_values ) ) {
							$old_replace_values[] = addcslashes( wcwm_url_scheme( $url, $old_schemes[ $i ] ), '/' );
							$new_replace_values[] = addcslashes( wcwm_url_scheme( home_url(), $new_schemes[ $i ] ), '/' );
						}
					}
		...
		// Get site URL
		$site_url = get_option( WCWM_SITE_URL );

		// Get home URL
		$home_url = get_option( WCWM_HOME_URL );

		// Get secret key
		$secret_key = get_option( WCWM_SECRET_KEY );

		// Get HTTP user
		$auth_user = get_option( WCWM_AUTH_USER );

		// Get HTTP password
		$auth_password = get_option( WCWM_AUTH_PASSWORD );

		$old_table_prefixes = array();
		$new_table_prefixes = array();

		// Set main table prefixes
		$old_table_prefixes[] = wcwm_servmask_prefix( 'mainsite' );
		$new_table_prefixes[] = wcwm_table_prefix();

		// Set site table prefixes
		foreach ( $blogs as $blog ) {
			if ( wcwm_main_site( $blog['Old']['BlogID'] ) === false ) {
				$old_table_prefixes[] = wcwm_servmask_prefix( $blog['Old']['BlogID'] );
				$new_table_prefixes[] = wcwm_table_prefix( $blog['New']['BlogID'] );
			}
		}

		// Set base table prefixes
		foreach ( $blogs as $blog ) {
			if ( wcwm_main_site( $blog['Old']['BlogID'] ) === true ) {
				$old_table_prefixes[] = wcwm_servmask_prefix( 'basesite' );
				$new_table_prefixes[] = wcwm_table_prefix( $blog['New']['BlogID'] );
			}
		}
		...
		// Set table prefixes
		$old_table_prefixes[] = wcwm_servmask_prefix();
		$new_table_prefixes[] = wcwm_table_prefix();
		...
		if ( isset( $config['Plugin']['Version'] ) && ( $version = $config['Plugin']['Version'] ) ) {
			if ( $version !== 'develop' && version_compare( $version, '4.10', '<' ) ) {
				$mysql->set_include_table_prefixes( array( wcwm_table_prefix() ) );
				$mysql->flush();
			}
		}

		// Set atomic tables (do not stop the current request for all listed tables if timeout has been exceeded)
		$mysql->set_atomic_tables( array( wcwm_table_prefix() . 'options' ) );

		// Set Visual Composer
		$mysql->set_visual_composer( wcwm_validate_plugin_basename( 'js_composer/js_composer.php' ) );

		// Set BeTheme Responsive
		$mysql->set_betheme_responsive( wcwm_validate_theme_basename( 'betheme/style.css' ) );

		// Import database
		if ( $mysql->import( wcwm_database_path( $params ), $query_offset ) ) {

			// Set progress
			Wcwm_Status::info( __( 'Done restoring database.', WCWM_PLUGIN_NAME ) );
			...
			$total_queries_size = wcwm_database_bytes( $params );
		...
		// Activate plugins
		wcwm_activate_plugins( wcwm_active_servmask_plugins() );

		// Set the new site URL
		update_option( WCWM_SITE_URL, $site_url );

		// Set the new home URL
		update_option( WCWM_HOME_URL, $home_url );

		// Set the new secret key value
		update_option( WCWM_SECRET_KEY, $secret_key );

		// Set the new HTTP user
		update_option( WCWM_AUTH_USER, $auth_user );

		// Set the new HTTP password
		update_option( WCWM_AUTH_PASSWORD, $auth_password );
