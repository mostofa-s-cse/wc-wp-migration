...
class Wcwm_Import_Validate {

	public static function execute( $params ) {

		// Verify file if size > 2GB and PHP = 32-bit
		if ( ! wcwm_is_filesize_supported( wcwm_archive_path( $params ) ) ) {
		...
		if ( isset( $params['total_archive_size'] ) ) {
			...
			$total_archive_size = wcwm_archive_bytes( $params );
		}
		...
		// Set progress
		Wcwm_Status::info( sprintf( __( 'Unpacking archive...<br />%d%% complete', WCWM_PLUGIN_NAME ), $progress ) );
		...
		if ( ! $archive->is_valid() ) {
			throw new Wcwm_Import_Exception(
				__(
					...
					'<a href="https://help.wc.com/knowledgebase/corrupted-archive/" target="_blank">this article</a> to resolve the problem.',
					WCWM_PLUGIN_NAME
				)
			);
		}

		$allowed_size = apply_filters( 'wcwm_max_file_size', WCWM_MAX_FILE_SIZE );
		...
		if ( $archive->has_not_reached_eof() ) {
			...
			if ( ( $completed = $archive->extract_by_files_array( wcwm_storage_path( $params ), array( WCWM_PACKAGE_NAME, WCWM_MULTISITE_NAME, WCWM_DATABASE_NAME ), array(), $file_bytes_written, $file_bytes_offset ) ) ) {
				$file_bytes_offset = 0;
			}
		...
		if ( $archive->has_reached_eof() ) {
			...
			if ( false === is_file( wcwm_package_path( $params ) ) ) {
				throw new Wcwm_Import_Exception(
					__(
						'Please make sure that your file was exported using <strong>WC WP Migration</strong> plugin. ' .
			...
			// Set progress
			Wcwm_Status::info( __( 'Done unpacking archive.', WCWM_PLUGIN_NAME ) );
